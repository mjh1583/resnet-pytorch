...
# 경사하강법을 사용하여 이 파라매터들을 최적화(optimize)합니다.
...
# 하이퍼파라매터를 설정한 뒤에는 최적화 단계를 통해 
# 모델을 학습하고 최적화할 수 있습니다. 
...
# 학습 중에 이 값을 최소화하려고 합니다. 
# 주어진 데이터 샘플을 입력으로 계산한 예측과 정답(label)을 비교하여 손실(loss)을 계산합니다.

# 일반적인 손실함수에는 회귀 문제(regression task)에 사용하는 
# nn.MSELoss(평균 제곱 오차(MSE; Mean Square Error))나 
# 분류(classification)에 사용하는 nn.NLLLoss (음의 로그 우도(Negative Log Likelihood)), 
# 그리고 nn.LogSoftmax와 nn.NLLLoss를 합친 nn.CrossEntropyLoss 등이 있습니다.
