...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, planes, stride=1):
        ...
        # 3*3 필터를 사용(너비와 높이를 줄일 때는 stride 값 조절 : 2로 하면 절반으로 줄어듦)
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)  # 배치 정규화(batch normalization)
        
        # 3*3 필터를 사용(패딩을 1만큼 주기 때문에 너비와 높이가 동일)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if batch_idx % 100 == 0:
            ...
            print("Current begin train loss : ", loss.item())
            
        print("\nTotal begin train accuracy : ", 100 * correct / total)
        print("Total begin train loss : ", train_loss)
