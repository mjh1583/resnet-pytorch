...
# torch.cuda 를 사용할 수 있는지 확인하고 그렇지 않으면 CPU를 계속 사용합니다.
...
# 이를 device 로 이동한 뒤, 구조(structure)를 출력합니다.
...
# 모델에 입력을 호출하면 각 분류(class)에 대한 
# 원시(raw) 예측값이 있는 10-차원 텐서가 반환됩니다. 
...
# 이를 설명하기 위해, 
# 28x28 크기의 이미지 3개로 구성된 미니배치를 가져와, 
# 신경망을 통과할 때 어떤 일이 발생하는지 알아보겠습니다.
input_image = torch.rand(3,28,28)
print(input_image.size())

# nn.Flatten
# nn.Flatten 계층을 초기화하여 각 28x28의 2D 이미지를 
...
# nn.Softmax
# 신경망의 마지막 선형 계층은 nn.Softmax 모듈에 전달될 
